...
def lambda_to_period(lambda_resonance):
    ...
    """
    period = (lambda_resonance - 0.63685) / 2.784
...
tt3.Layout(core_width=0.4,
              cladding_width=0.4 + 7)

tt4 = pdk.SiWireWaveguideTemplate()
tt4.Layout(core_width=0.3,
              cladding_width=0.3 + 7)
...
# class FanoutBend(i3.PCell):
#     _name_prefix = "FanoutBend"
#     class Layout(i3.LayoutView):
...
#             bend_radius = 10
#             insts_insts = {
#                 'TempPort_low': TempPort(),
#                 'TempPort_high': TempPort()
#             }
#             insts_specs = [
#                 i3.PlaceRelative('TempPort_high:out', 'TempPort_low:in',
#                                  (gap_horizontal, gap_vertical)),
#                 i3.ConnectBend.connect(start_port='TempPort_low:out', end_port='TempPort_high:in',
...
class GACDCPhaseShiftApodized(i3.PCell):
    ...
    class Layout(i3.LayoutView):
        ...
        def _generate_elements(self, elems):
            ...
            # add gratings
            # upper side of the bus waveguide, apply misalignment
            ...
            # downside of the drop waveguide
            for i in range(int(num_period)):
                elems += i3.Rectangle(
                    layer=core_layer,
                    center=(20 + (0.25 + i) * grating_period,
            ...
            # upper side of the input waveguide
            if num_side == 2:
                for i in range(int(num_period)):
                    elems += i3.Rectangle(
                        layer=core_layer,
...
class GACDCFoundry(i3.PCell):
    ...
    w_bus = i3.PositiveNumberProperty(default=0.4, doc="Width of the bus waveguide, act as input.")
    w_drop = i3.PositiveNumberProperty(default=0.3, doc="Width of the drop waveguide, act as output")
    gap = i3.PositiveNumberProperty(default=0.75, doc="The gap between two waveguides.")
    grating_width = i3.PositiveNumberProperty(default=0.15, doc="The gap between two waveguides.")
    lambda_resonance = i3.PositiveNumberProperty(default=1.55, doc="desired resonance lambda.")
    grating_period = i3.PositiveNumberProperty(default=0.38, doc="The period of the GACDC.")
    grating_pos_up = i3.PositiveNumberProperty(default=0.15, doc="The period of the GACDC.")
    grating_pos_down = i3.PositiveNumberProperty(default=0.15, doc="The period of the GACDC.")
    num_period = i3.PositiveNumberProperty(default=1200, doc="The number of periods i.e. the coupling length.")
    num_side = i3.PositiveNumberProperty(default=1, doc="on how many sides of the waveguide.")
    apodization_function = i3.StringProperty(default='Gaussian',
                                             doc="Apodization function: Gaussian")
    apodization_strength = i3.PositiveNumberProperty(default=12, doc="apodization strength.")
    ...
    class Layout(i3.LayoutView):
        ...
        def _generate_elements(self, elems):
            ...
            apodization_function = self.apodization_function
            apodization_strength = self.apodization_strength
            grating_pos_up = self.grating_pos_up
            grating_pos_down = self.grating_pos_down
            """
            Si core
            """
            # 'add' port
                ...
                elems += i3.Rectangle(
                    ...
                    center=(20 + (0.25 + i) * grating_period +
                            grating_period / 2 * (1 - mis_alignment(i, num_period, apodization_function,
                                                                    apodization_strength)),
                            -grating_pos_down),
                    box_size=(grating_period / 2, grating_width),
                )
            # down of the bus waveguide, apply misalignment
            # for i in range(int(num_period)):
            #     elems += i3.Rectangle(
...
tt1.Layout(core_width=0.4)
tt2 = pdk.SiWireWaveguideTemplate()
tt2.Layout(core_width=0.3)


class GACDCFoundryArm(i3.PCell):
    """
    bandpass filter based on grating assisted contra-directional coupler (GACDC)
    apply phase-shift apodization
    ...
    class Layout(i3.LayoutView):
        def _generate_instances(self, insts):
            ...
            insts_insts = {
                'GACDC': GACDCFoundry(grating_period=self.grating_period,
                                      num_period=self.num_period,
                                      grating_width=self.grating_width,
                                      grating_pos_up=self.grating_pos_up,
                                      grating_pos_down=self.grating_pos_down,
                ...
                'TempPort': TempPort(),
                'Taper_in': Taper_TempPort1(width=0.4),
                'Taper_through': Taper_TempPort1(width=0.4),
                'Taper_add': Taper_TempPort2(width=0.3),
                'Taper_drop': Taper_TempPort2(width=0.3),
                'Port2': TempPort2(),
                # 'inarm': bend_in_out,
                # 'outarm': bend_in_out,
...
class GACDCEblComp(i3.PCell):
    ...
    apodization_function = i3.StringProperty(default='Gaussian',
                                             doc="Apodization function: Gaussian")
    apodization_strength = i3.PositiveNumberProperty(default=12, doc="apodization strength.")
    width_compensation_bus = i3.NonNegativeNumberProperty(default=0,
                                                          doc="width difference between up and under side of grating "
                                                              "tooth")
    width_compensation_drop = i3.NonNegativeNumberProperty(default=0,
                                                           doc="width difference between up and under side of grating "
                                                               "tooth")
    ...
    class Layout(i3.LayoutView):
        ...
        def _generate_elements(self, elems):
            width_compensation_bus = self.width_compensation_bus
            width_compensation_drop = self.width_compensation_drop
            grating_width_bus_waveguide = self.grating_width_bus_waveguide
            grating_width_drop_waveguide = self.grating_width_drop_waveguide
            w_bus = self.w_bus
            w_bus_port = w_bus + grating_width_bus_waveguide
            ...
            for i in range(int(num_period)):
                elems += i3.Wedge(
                    layer=core_layer,
                    begin_coord=(20 + (0.25 + i) * grating_period +
                                 grating_period / 2 * (1 - mis_alignment(i, num_period, apodization_function,
                                                                         apodization_strength)),
                                 -gap / 2 + grating_width_bus_waveguide),
                    end_coord=(20 + (0.25 + i) * grating_period +
                               grating_period / 2 * (1 - mis_alignment(i, num_period, apodization_function,
                                                                       apodization_strength)),
                               -gap / 2),
                    begin_width=grating_period / 2 + width_compensation_bus,
                    end_width=grating_period / 2 - width_compensation_bus,
                )
            # down of the bus waveguide, apply misalignment
            for i in range(int(num_period)):
                elems += i3.Wedge(
                    ...
                    begin_coord=(20 + (0.25 + i) * grating_period +
                                 grating_period / 2 * (1 - mis_alignment(i, num_period, apodization_function,
                                                                         apodization_strength)),
                                 -gap / 2 - w_bus - grating_width_bus_waveguide),
                    end_coord=(20 + (0.25 + i) * grating_period +
                               grating_period / 2 * (1 - mis_alignment(i, num_period, apodization_function,
                                                                       apodization_strength)),
                               -gap / 2 - w_bus),
            ...
            for i in range(int(num_period)):
                elems += i3.Wedge(
                    layer=core_layer,
                    begin_coord=(20 + (0.25 + i) * grating_period,
                                 gap / 2 + w_drop + grating_width_drop_waveguide),
                    end_coord=(20 + (0.25 + i) * grating_period,
                               gap / 2 + w_drop),
                    begin_width=grating_period / 2 + width_compensation_drop,
                    end_width=grating_period / 2 - width_compensation_drop,
                )
            # down of the drop waveguide
            for i in range(int(num_period)):
                elems += i3.Wedge(
                    layer=core_layer,
                    begin_coord=(20 + (0.25 + i) * grating_period,
                                 gap / 2 - grating_width_drop_waveguide),
                    end_coord=(20 + (0.25 + i) * grating_period,
                               gap / 2),
                    begin_width=grating_period / 2 + width_compensation_drop,
                    end_width=grating_period / 2 - width_compensation_drop,
...
class GACDC_HT_Foundry(i3.PCell):
    """
    ports:2x2 端口设置示意如下（下面的是宽波导，宽度600nm，上面是窄波导，宽度350nm）
    ...
    class Layout(i3.LayoutView):
        def _generate_instances(self, insts):
            ...
            insts_insts = {
                'GACDC': GACDCFoundryArm(gap=self.gap,
                                         grating_period=self.grating_period,
                                         num_period=self.num_period,
...
class GACDC_HT_ScanFieldTwoStage(i3.PCell):
    """
    adjust port length to EBL scan field
    “GACDCPhaseShiftApodized”的端口设置
    ports:2x2 端口设置示意如下（下面的是宽波导，宽度600nm，上面是窄波导，宽度350nm）
    →   add        ---------------------------------    drop  →（滤波输出端口）
    ←   through    ---------------------------------    in    ←（输入端口）
    cascaded GACDC
    ...
    ←   drop    ---------------------------------    add   ←
    """
    _name_prefix = "GACDC_HT_ScanFieldTwoStage"
    EBL_scan_filed_length = i3.PositiveNumberProperty(default=1000, doc="gap between components")
    grating_period = i3.PositiveNumberProperty(default=0.33, doc="The period of the GACDC.")
    heater_open = i3.BoolProperty(default=True, doc='False for no heater')
    lambda_resonance = i3.PositiveNumberProperty(default=1.55, doc="desired resonance lambda.")
    ...
    class Layout(i3.LayoutView):
        def _generate_instances(self, insts):
            ...
            EBL_scan_filed_length = self.EBL_scan_filed_length
            insts_insts = {
                'GACDC_1': GACDCPhaseShiftApodized(lambda_resonance=self.lambda_resonance),
                'GACDC_2': GACDCPhaseShiftApodized(lambda_resonance=self.lambda_resonance),
                # 'TempPort_in': TempPort(),
                # 'TempPort_drop': TempPort(),
                # 'TempPort_through': TempPort(),
                # 'TempPort_add': TempPort(),
            ...
            insts_specs = [
                i3.FlipV('GACDC_1'),
                i3.PlaceRelative('GACDC_2:drop', 'GACDC_1:drop',
                                 (0, -30)),
                # i3.Place('TempPort_in:in', (0, 0)),
                # i3.PlaceRelative('Sbend_in:in', 'TempPort_in:in',
                # (EBL_scan_filed_length / 20, 0)),
                i3.FlipV('Sbend_in'),
                i3.FlipV('Sbend_add'),
                i3.PlaceRelative('Sbend_in:out', 'GACDC_1:through', (0, 0)),
                i3.PlaceRelative('Sbend_drop:out', 'GACDC_2:through', (0, 0)),
                i3.PlaceRelative('Sbend_through:in', 'GACDC_1:in', (0, 0)),
                i3.PlaceRelative('Sbend_add:in', 'GACDC_2:in', (0, 0)),
                # i3.PlaceRelative('GACDC:drop', 'TempPort_drop:in', (0, 0)),
                # i3.PlaceRelative('TempPort_add:in', 'Sbend_drop:in',
                # #                  (EBL_scan_filed_length - EBL_scan_filed_length / 20, 0)),
                # i3.PlaceRelative('TempPort_through:in', 'TempPort_in:in',
                #                  (EBL_scan_filed_length, 0)),
                i3.ConnectManhattan([
                    ('GACDC_2:add', 'GACDC_1:add'),
                    ('GACDC_2:drop', 'GACDC_1:drop'),
                ])
            ]
            # if self.heater_open:
            #     insts_insts['heater'] = custom_elec_single(m1_taper_width=10, )
...
class GACDCFoundryArmFourChannel(i3.PCell):
    """
...
if __name__ == '__main__':
    # pass
    # # lo_1 = GACDCFoundry(gap=0.75,
    #                     grating_period=0.375,
    #                     num_period=2250)
    # lo_test = GACDCFoundryArm(num_period=500,
    #                        num_side=2,
    #                        grating_width_bus_waveguide=0.18,
    #                        grating_width_drop_waveguide=0.18,
    #                        w_bus=0.4,
    #                        w_drop=0.3,
    #                        gap=0.7
    #                        )
    lo2 = GACDCFoundryArm()
    lo2.Layout().write_gdsii('GACDCFoundryArm.gds')
    # lo_test = GACDCFoundryArm()
    # # lo_test.Layout().visualize(annotate=True)
    # lo_test.Layout().write_gdsii('GACDCFoundryArm.gds')
    # lo1 = GACDCFoundryArmFourChannel()
    # lo1.Layout().write_gdsii('GACDCFoundrArmFourChannel.gds')
    # lo_1 = FanoutBend()
    # lo_2 = GACDCEblComp(width_compensation_bus=0.015,
    #                     width_compensation_drop=0.01,
    #                     num_period=1000)
    # lo_3 = GACDCEblComp(width_compensation_bus=0.02,
    #                     width_compensation_drop=0.015,
    #                     num_period=1000)
    # lo_4 = GACDCEblComp(width_compensation_bus=0.025,
    #                     width_compensation_drop=0.02,
    #                     num_period=1000)
    # lo = GACDC_HT_ScanField()
    # lo = GACDC_HT()
    # lo = GACDCFourChannel()
    # lo_1.Layout().visualize(annotate=True)
    # lo_1.Layout().write_gdsii('GACDCFoundry_3DFDTD_gap0.75-1000periods.gds')
    # lo_2.Layout().write_gdsii('GACDCEblComp_2.gds')
    # lo_3.Layout().write_gdsii('GACDCEblComp_3.gds')
    # lo_4.Layout().write_gdsii('GACDCEblComp_4.gds')
